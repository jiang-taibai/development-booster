  ...
  methods: {
    ...
    confirmModification() {
      ...
      for (const scope of scopes) {
        ...
        if (majorRegistry) {
          commands.push(this.autoCommandPrefix(this.pkgData.path) + ' config set global.index-url ' + majorRegistry);
        }
        let url = new URL(majorRegistry);
        if (url.protocol === 'http') {
          commands.push(this.autoCommandPrefix(this.pkgData.path) + ' config set global.trusted-host ' + url.hostname);
        }
        let minorRegistryTrustedHosts = [];
        if (minorRegistries && minorRegistries.length > 0) {
          commands.push(this.autoCommandPrefix(this.pkgData.path) + ' config set global.extra-index-url ' + minorRegistries.join(" "));
          ...
          if (minorRegistryTrustedHosts.length > 0) {
            commands.push(this.autoCommandPrefix(this.pkgData.path) + ' config set global.extra-trusted-host ' + minorRegistryTrustedHosts.join(" "));
          }
        }
      }
